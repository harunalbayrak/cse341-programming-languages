...
(defun check (*my-string* my-stream)
    ...
    (loop for x from 0 to *length-1a* do
        ...
        (check-string-p (nth x *list*) "LIST" "KW_LIST" my-stream 4)
        (check-string-p (nth x *list*) "DEFFUN" "KW_DEFFUN" my-stream 4)
        (check-string-p (nth x *list*) "AND" "KW_AND" my-stream 4)
        (check-string-p (nth x *list*) "OR" "KW_OR" my-stream 4)
        (check-string-p (nth x *list*) "NOT" "KW_NOT" my-stream 4)
        (check-string-p (nth x *list*) "EQUAL" "KW_EQUAL" my-stream 4)
        (check-string-p (nth x *list*) "LESS" "KW_LESS" my-stream 4)
        (check-string-p (nth x *list*) "NIL" "KW_NIL" my-stream 4)
        (check-string-p (nth x *list*) "APPEND" "KW_APPEND" my-stream 4)
        (check-string-p (nth x *list*) "CONCAT" "KW_CONCAT" my-stream 4)
        (check-string-p (nth x *list*) "SET" "KW_SET" my-stream 4)
        (check-string-p (nth x *list*) "FOR" "KW_FOR" my-stream 4)
        (check-string-p (nth x *list*) "IF" "KW_IF" my-stream 4)
        (check-string-p (nth x *list*) "EXIT" "KW_EXIT" my-stream 4)
        (check-string-p (nth x *list*) "LOAD" "KW_LOAD" my-stream 4)
        (check-string-p (nth x *list*) "DISP" "KW_DISP" my-stream 4)
        (check-string-p (nth x *list*) "TRUE" "KW_TRUE" my-stream 4)
        (check-string-p (nth x *list*) "FALSE" "KW_FALSE" my-stream 4)
        (check-string-p (nth x *list*) "(" "OP_OP" my-stream 4)
        (check-string-p (nth x *list*) ")" "OP_CP" my-stream 4)
        (check-string-p (nth x *list*) "+" "OP_PLUS" my-stream 4)
        (check-string-p (nth x *list*) "-" "OP_MINUS" my-stream 4)
        (check-string-p (nth x *list*) "/" "OP_DIV" my-stream 4)
        (check-string-p (nth x *list*) "*" "OP_MULT" my-stream 4)
        (check-string-p (nth x *list*) "**" "OP_DBLMULT" my-stream 4)
        (check-string-p (nth x *list*) "“" "OP_OC" my-stream 4)
        (check-string-p (nth x *list*) "”" "OP_CC" my-stream 4)
        (check-string-p (nth x *list*) "," "OP_COMMA" my-stream 4)
        (check-string-p (nth x *list*) "0" "VALUE" my-stream 4)
...
(with-open-file (my-stream 
    ...
    (if (equal (nth 0 *args*) NIL) 
        (progn
            (loop
                do (defparameter *my-string* (read-line))

                (check *my-string* my-stream)
            )
        )
        (progn
